...
export default function ViewAppointmentScreen() {
	...
	const { fetchCanceledAppointment, isLoading } = useAppointment();

	const dataISO = viewAppointment.date;
	const date = new Date(dataISO);

	function canCancelAppointment(appointmentDateISO: string) {
		const appointmentDate = new Date(appointmentDateISO);
		const now = new Date();

		const limitToCancel = new Date(
			appointmentDate.getTime() - 2 * 60 * 60 * 1000,
		);

		return now < limitToCancel;
	}
	async function HandelClickCancelAppointment(id: number) {
		const canceledToAppointment = canCancelAppointment(viewAppointment.date);

		if (!canceledToAppointment) {
			showMessage({
				message:
		...
		await fetchCanceledAppointment(id);
	...
	return (
		<ContainerDefault key={viewAppointment.id}>
			<HeaderDefault paddingTop={40}>
				...
				<HeaderTitle marginTop={56}>
					{viewAppointment.service?.name}
				</HeaderTitle>
			</HeaderDefault>
			<View style={{ marginTop: 32 }}>
				<Text fontFamily="bold" fontSize="h3">
					{finalFormatted}
				</Text>
			...
			<Button
				disabled={isLoading}
				onPress={() => HandelClickCancelAppointment(viewAppointment.id)}
				alignItems="center"
				borderWidth={1}
				borderColor="text"
				marginTop={112}
				...
				borderRadius={18}
			>
				<ButtonText weight="medium" color="text" fontSize="lg">
