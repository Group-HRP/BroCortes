...
interface ButtonProps {
	...
	fontSize?:
		| keyof (typeof ThemeType)["fonts"]["sizesBody"]
		| keyof (typeof ThemeType)["fonts"]["sizesHeading"];
	backgroundColor?: keyof (typeof ThemeType)["colors"];
	color?: keyof (typeof ThemeType)["colors"];
	fontFamily?:
		| keyof (typeof ThemeType)["fonts"]["body"]
		| keyof (typeof ThemeType)["fonts"]["heading"];
	margin?: number;
...
export const Button = styled.TouchableOpacity<ButtonProps>`
  ...
  ${({ borderRadius, theme }) => {
		...
		return `border-radius: ${
			theme.fonts.sizesBody[
				borderRadius as keyof typeof theme.fonts.sizesBody
			] ||
			theme.fonts.sizesHeading[
				borderRadius as keyof typeof theme.fonts.sizesHeading
			] ||
			4
		}px;`;
	}}
...
export const ButtonText = styled.Text<ButtonTextProps>`
  ...
  font-size: ${({ fontSize = "medium", theme }) =>
		...
		theme.fonts.sizesHeading[
			fontSize as keyof typeof theme.fonts.sizesHeading
		]}px;
